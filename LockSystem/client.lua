...
local keyParam = Keys["Y"] -- e.g : Keys["H"] will be change the U key to the H key for lock/unlock a vehicle
...
AddEventHandler("ls:lock", function(lockStatus, vehicle)
	...
	if lockStatus == 1 or lockStatus == 0 then -- Si le véhicule est déverrouillé (on le verrouille):
		...
		-- ## Notifications
			if soundEnable then TriggerServerEvent("InteractSound_SV:PlayWithinDistance", soundDistance, "Fermer à clé", 1.0) end
			TriggerEvent("ls:sendNotification", notificationParam, "Véhicule verrouillé.", 0.080)
		-- ## Notifications
	...
	elseif lockStatus == 2 then -- Si le véhicule est verrouillé
		...
		-- ## Notifications
			if soundEnable then	TriggerServerEvent("InteractSound_SV:PlayWithinDistance", soundDistance, "ouvrir", 1.0) end
			TriggerEvent("ls:sendNotification", notificationParam, "Véhicule déverrouillé.", 0.080)
		-- ## Notifications
...
AddEventHandler("ls:notify", function(text, time)
	...
	Citizen.InvokeNative(0x1E6611149DB3DB6B, "CHAR_MP_FM_CONTACT", "CHAR_MP_FM_CONTACT", true, 1, "Véhicule", "", time)
...
AddEventHandler("ls:sendNotification", function(param, message, duration)
	...
	elseif param == 2 then
		TriggerEvent('chatMessage', 'Véhicule', { 255, 128, 0 }, message)
	end
end)
