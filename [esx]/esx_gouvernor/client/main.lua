...
function OpenGouvernorMenu()
	...
	if PlayerData.job ~= nil and PlayerData.job.name == 'gouvernor' and PlayerData.job.grade_name == 'boss' then
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'gouvernor',
		{
			title    = _U('gouvernor'),
			elements = elements
		},
				...
				ESX.UI.Menu.Open(
					...
					function(data, menu)
						...
						if amount == nil then
							...
							menu.close()
							TriggerServerEvent('esx_society:withdrawMoney', 'gouvernor', amount)
						end
				...
				ESX.UI.Menu.Open(
					...
					function(data, menu)
						...
						if amount == nil then
							...
							menu.close()
							TriggerServerEvent('esx_society:depositMoney', 'gouvernor', amount)
						end
				...
				ESX.UI.Menu.Open(
					...
					function(data, menu)
						...
						if amount == nil then
							...
							menu.close()
							TriggerServerEvent('esx_society:washMoney', 'gouvernor', amount)
						end
...
AddEventHandler('esx_gouvernorjob:hasEnteredMarker', function(zone)
	if zone == 'OfficeActions' and PlayerData.job ~= nil and PlayerData.job.name == 'gouvernor' then
		CurrentAction     = 'gouvernor_menu'
		CurrentActionMsg  = _U('press_to_access')
		CurrentActionData = {}
	end
...
AddEventHandler('esx_gouvernorjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)
	...
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and (GetGameTimer() - GUI.Time) > 300 then
				...
				if CurrentAction == 'gouvernor_menu' then
					OpenGouvernorMenu()
				end
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('esx_gouvernorjob:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('esx_gouvernorjob:hasExitedMarker', LastZone)
		end
